  ...
  try {
    ...
    if (existingCount > 0 && !clearExisting) {
      return NextResponse.json({
        message:
          'Sample data already exists. Send { "clearExisting": true } to replace existing data.',
        count: existingCount,
      });
    }
    ...
    if (clearExisting && existingCount > 0) {
      ...
      console.log(
        `Cleared ${existingCount} existing timesheets for user ${userId}`
      );
    }
    ...
    const statusCounts = createdTimesheets.reduce(
      (acc: Record<string, number>, timesheet) => {
    ...
    return NextResponse.json(
      {
        message: clearExisting
          ? "Sample data replaced successfully"
